...
- Non-critical cloud environments
...
RubixKube includes multiple layers of safety guardrails to protect your environments:

<CardGroup cols={2}>
  <Card title="Observe-Only by Default" icon="eye">
    Default mode is **read-only observation**. Any action that modifies environment state requires explicit approval.
  </Card>
  <Card title="Minimal Permissions" icon="key">
    RubixKube follows **least privilege**. Kubernetes installs do not require cluster-admin access.
  </Card>
...
<AccordionGroup>
  <Accordion title=" Data We Collect" icon="check">
    ...
    - **Infrastructure metadata** - resource identifiers and environment context
    - **Metrics** - CPU, memory, network usage, health checks
    - **Events** - infrastructure and platform state changes
    - **Resource status** - workload or service health
...
**Privacy First:** We only collect what's necessary for reliability monitoring.
  ...
  <Accordion title=" Security Best Practices" icon="lock">
    **For Beta Testing:**
    ...
    - Avoid environments with PII or regulated data
    - Redact sensitive information before sharing logs with support
    - Review RubixKube observer permissions regularly
...
<AccordionGroup>
  ...
  <Accordion title=" You May Encounter">
    ...
    - Edge cases in complex multi-environment setups
    - Performance variations on very large environments
...
- Environment type (Kubernetes, AWS, GCP, VM)
- Platform/version details
- RubixKube version
- Reproduction steps
- Relevant observer logs
