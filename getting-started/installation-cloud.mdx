---
title: 'Cloud Installation (Kubernetes, Beta)'
description: 'Install RubixKube on a cloud Kubernetes environment - AWS EKS, Google GKE, or Azure AKS'
---

Install RubixKube on your **cloud Kubernetes environment** to test our Site Reliability Intelligence platform in a cloud setup.

<Info>
Need AWS or GCP account-level observation from a VM instead of cluster deployment? Use [Environment Quick Starts](/getting-started/environment-quickstarts).
</Info>

<Warning>
<strong> Beta Release Notice</strong>

RubixKube is currently in <strong>Beta</strong>. While you can install it on cloud Kubernetes environments for <strong>testing and evaluation</strong>, it is <strong>NOT recommended for production environments</strong> yet.
...
<Info>
<strong>Why Install on Cloud Kubernetes?</strong>

Testing RubixKube on a cloud Kubernetes environment gives you a more realistic experience compared to local KIND, including real cloud resources, load balancers, and persistent storage.
</Info>

---

## Supported Cloud Kubernetes Platforms
...
RubixKube deploys two in-cluster components:
- **rubixkube-observer** — Event collection and monitoring
- **kubernetes-mcp-server** — Kubernetes API integration

**Typical resource footprint during normal operation:**
- **Memory:** ~255Mi combined (observer ~146Mi, MCP server ~109Mi)
- **CPU:** Under 10 millicores combined (typically ~7m)
