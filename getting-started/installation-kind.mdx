...
<Info>
  Complete Beginner?
   ...
   No problem! We'll walk you through installing everything from scratch, even if you've never used Docker or Kubernetes before.
</Info>
...
<Note>
  Estimated Time:
   ...
   20-30 minutes for first-time installation (including downloads)
</Note>
  ...
  <Tab title="macOS">
    ...
    <Tip>
      First Time?
       ...
       Docker may ask for system permissions. Click "OK" to allow it.
    </Tip>
  </Tab>
  <Tab title="Windows">
    ...
    **Prerequisites:** - Windows 10 64-bit: Pro, Enterprise, or Education (Build 19041 or higher)
    ...
    <Warning>
      Virtualization Required:
       ...
       Docker Desktop requires hardware virtualization. Enable VT-x/AMD-V in your BIOS if you see errors.
    </Warning>
  </Tab>
  <Tab title="Linux">
...
<Tip>
  Pro Tip:
   ...
   The cluster automatically sets itself as your active Kubernetes context, so kubectl commands will target it.
</Tip>
...
<Note>
  Troubleshooting:
   ...
   If the node shows "NotReady", wait 30 seconds and check again. It takes time to initialize.
</Note>
...
<Warning>
  Important:
   ...
   Use YOUR specific command from the dashboard - it contains your unique API key!
</Warning>
...
<Tip>
  What just happened?
   ...
   RubixKube installed an "observer" agent in your cluster that monitors your infrastructure and reports back to the RubixKube platform.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title=" What if STATUS shows 'CrashLoopBackOff'?">
    ...
    **Check logs to see why:** \`\`\`bash kubectl logs -n rubixkube deployment/rubixkube-observer
...
<Tip>
  Tip:
   ...
   If you see steady log output with [INFO] messages, everything is working correctly!
</Tip>
...
<Note>
  Note:
   ...
   It may take 1-2 minutes for your cluster to appear in the dashboard after the pod starts running.
</Note>
...
```bash docker start rubixkube-local-control-plane
...
<AccordionGroup>
  ...
  <Accordion title="KIND cluster creation fails">
    ...
    **Solutions:**    1. Ensure Docker has enough resources (4GB+ RAM recommended) 2. Check Docker is running: `docker ps` 3. Delete any existing cluster: `kind delete cluster --name rubixkube-local` 4. Try creating again
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Try Breaking Things" icon="flask" href="/tutorials/break-pod-image-error">
    See RubixKube detect and fix issues automatically
  </Card>
  <Card title="Deploy to Production" icon="cloud" href="/getting-started/installation-cloud">
    Ready for production? Install on a real cloud cluster
  </Card>
  <Card title="Chat with Your Infrastructure" icon="comments" href="/tutorials/talk-to-infra">
...
<Tip>
  Best Practice:
   ...
   Start with KIND to learn RubixKube, then move to cloud for production workloads.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Support" icon="envelope">
    Email us at [connect@rubixkube.ai](mailto:connect@rubixkube.ai)
  </Card>
...
<Note>
  Stuck?
   ...
   Don't hesitate to reach out. We're here to help you get started with RubixKube!
</Note>