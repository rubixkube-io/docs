...
| `/connect` | `<host \| user@host>` | Connects to a machine so Kepler can work there: a machine it already knows, by name or prefix (`/connect pi5`), or a new `user@host`. A terminal tab opens only if the host asks you for something. |
