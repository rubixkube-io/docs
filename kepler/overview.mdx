...
{/* SCREENSHOT: kepler/hero-investigation.png : state: the Kepler home screen, light theme, sidebar with workspaces, the greeting, the composer in Observe with a suggested prompt that picks up yesterday's checkout investigation */}
<Frame>
  <img src="/images/kepler/hero-investigation.png" alt="The Kepler home screen. The sidebar lists New session, New war room, Search, Feed, Automations, Memory and Topology, then the workspaces. In the middle, a greeting by name, the question What should we look into, and the composer in Observe with a suggested prompt: apply the checkout-config remediation patch in prod." />
