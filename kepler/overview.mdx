...
{/* SCREENSHOT: kepler/hero-investigation.png : state: an Observe session answering "Why is checkout crashlooping in prod?", with the kubectl get pods and kubectl describe pod cards expanded to show their output, and the Terminal panel open on the right */}
<Frame>
  <img src="/images/kepler/hero-investigation.png" alt="Kepler answering the question why checkout is crashlooping in prod. Cards for kubectl get pods and kubectl describe pod are expanded to show their raw output, followed by a Finding that the image tag does not exist, the Evidence it observed, and a Fix. The Terminal panel is open on the right." />
