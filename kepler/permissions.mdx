...
Leave the defaults on to start. Files and Shell are on Allow, which means reads run without a prompt and writes follow the conversation's posture. MCP servers are on Ask, so every call to an external integration shows you what it is about to send. Tighten or loosen one control at a time once you know what a normal session looks like.
...
Each switch is Allow, Ask or Block. Allow never means writes run freely: reads run without a prompt, and writes still follow the conversation's posture.

| Tool | Covers | Default | What Ask means |
|---|---|---|---|
| **Files** | Reading, listing and searching files in your workspace. | Allow | Every file read docks a card before it runs. |
| **Shell** | Commands Kepler runs in your terminal. | Allow | Every command asks, reads included. Set this if you want to approve even `kubectl get` for a while. |
