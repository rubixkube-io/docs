...
| **Always** | Runs it and allows that same verb prefix from now on, in every conversation. | A rule, permanently. It shows up in **Settings > Permissions** and you can delete it there. |
...
{/* SCREENSHOT: kepler/approval-dock.png : state: Assist posture, approval dock showing a kubectl label command with a server dry-run preview and the four buttons Reject, Always, This session, Run */}
<Frame>
  <img src="/images/kepler/approval-dock.png" alt="The approval dock above the composer showing a kubectl label command, a server dry-run preview reading deployment.apps/checkout not labeled, and four buttons: Reject, Always, This session, and Run." />
