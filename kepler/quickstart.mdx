...
<Steps>
  ...
  <Step title="Connect a model">
    ...
    {/* SCREENSHOT: kepler/model-picker.png : state: the model picker open over the composer, hosted Kepler models first, a BYO provider group below, the Reasoning slider and the Max mode toggle at the bottom */}
    <Frame>
      <img src="/images/kepler/model-picker.png" alt="The Kepler model picker open over the composer. Hosted Kepler models are listed first under your Kepler plan, then a provider group from your own plan, then Google Vertex AI under your API key. Below the list sit a Reasoning slider and a Max mode toggle for this session." />
  ...
  <Step title="Ask a read-only question">
    ...
    {/* SCREENSHOT: kepler/observe-answer.png : state: an Observe reply to "Why is checkout crashlooping in prod?": a row per kubectl command, a "Not run" row for the patch Observe refused, then Finding, Evidence and the start of the remediation. Posture chip reads Observe. */}
    <Frame>
      <img src="/images/kepler/observe-answer.png" alt="Kepler's reply in Observe to why checkout is crashlooping in prod. A row for each kubectl command it ran, a Not run row for a configmap patch it was not allowed to run, then a Finding that the ConfigMap provides DATABASE_URI while checkout needs DATABASE_URL, an Evidence list quoting the pod log, and a safe remediation." />
