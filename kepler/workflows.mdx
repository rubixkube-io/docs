...
{/* SCREENSHOT: kepler/observe-answer.png : state: an Observe reply to "Why is checkout crashlooping in prod?": a row per kubectl command, a "Not run" row for the refused patch, then Finding, Evidence and the start of the remediation */}
<Frame>
  <img src="/images/kepler/observe-answer.png" alt="A Kepler answer in Observe: a row for each kubectl command it ran, a Not run row for a patch Observe refused, then a Finding about a misnamed ConfigMap key, an Evidence list quoting the pod log and the deployment's change cause, and a safe remediation." />
