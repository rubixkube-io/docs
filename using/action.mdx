...
<Frame>
  <img
    src="/images/platform/actions-center.png"
    alt="Actions Center with total, in-progress, completed, and high-priority counters. Active, Waiting, Review, Completed, and Closed tabs. One action expanded showing RCA summary, details, timeline, and related incident."
  />
  <Caption>The Actions Center. Headline counters on top, status tabs for triage, and an expandable row per action that reveals the RCA summary, timeline, and the RCA report it links back to.</Caption>
</Frame>

**The header shows:**
- **Total Actions**: every remediation task ever created for this environment.
- **In Progress**: actions a human has started.
- **Completed**: actions that have finished verification.
- **High Priority**: urgent actions that need attention first.
